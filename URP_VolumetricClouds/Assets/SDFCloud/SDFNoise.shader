Shader "SDF/Noise" {
	...
	HLSLINCLUDE
		...
		float4 Fragement(Interpolators interpolators) : SV_Target
		{
			...
			depth = 0;
			// 本地空间的光照方向
			float3 localLightDirection = normalize(mul((float3x3)unity_WorldToObject, _MainLightPosition.xyz));
			...
			for(int i = 0; i < 20; ++i)
			{
				...
				if(sdf < 0 && density > 0)
				{
					...
					// 模拟环境光照
					float3 ambient = _MainLightColor * lerp((0.2), (0.8), (currentPos.y + 0.5)) * _AmbientIntensity;
					float3 currentColor = _MainLightColor * multipleOctaves(opticalDepth, cosTheta) * PI * phase * _SigmaT * density;
				...
				if(any(currentPos < -0.6) || any(currentPos > 0.6)) break;
			}
